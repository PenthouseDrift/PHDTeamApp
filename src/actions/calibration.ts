...
export async function createCalibration(
  ...
  const calibration: CalibrationSetup = {
    ...
    spurGear: parsed.data.spurGear,
    pinionGear: parsed.data.pinionGear,
    finalDriveRatio: parsed.data.finalDriveRatio,
...
export async function saveAdvisedCalibration(
  ...
  const calibration: CalibrationSetup = {
    ...
    spurGear: setup.spurGear ?? 0,
    pinionGear: setup.pinionGear ?? 0,
    finalDriveRatio: setup.finalDriveRatio ?? 0,
  ...
  for (const id of ids) {
    ...
    if (data && Object.keys(data).length > 0) {
      calibrations.push({
        ...
        spurGear: Number(data.spurGear) || 0,
        pinionGear: Number(data.pinionGear) || 0,
        finalDriveRatio: Number(data.finalDriveRatio) || 0,
