...
export function FeedbackAdminClient({
  ...
  const [activeTab, setActiveTab] = useState<"feedback" | "flagged">("feedback");
          ...
          {flaggedList.length === 0 ? (
            ...
            <div className="space-y-3">
              {flaggedList.map((item) => (
                <div
                  key={item.flaggedId}
                  className="rounded-2xl border border-red-500/30 bg-white dark:bg-zinc-900 p-5 space-y-3 shadow-xs"
                >
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      <span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-500/10 border border-red-500/30 px-2 py-0.5 rounded-full uppercase tracking-wider">
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-red-600 dark:hover:text-red-400 text-xs font-bold transition-colors"
                    >
                      {actionLoading === item.flaggedId ? "Dismissing..." : "Dismiss Incident"}
                    </button>
                  </div>

                  <div className="flex flex-col sm:flex-row gap-3 items-start bg-red-500/5 p-3 rounded-xl border border-red-500/20">
                    <a href={item.imageUrl} target="_blank" rel="noopener noreferrer" className="shrink-0">
                      <img
                        src={item.imageUrl}
                        alt="Flagged image"
                        className="w-24 h-24 rounded-lg object-cover border border-red-500/30 filter blur-xs hover:blur-none transition-all"
                      />
                    </a>
